...
public class RestEntityFactory implements V1Constants {
    ...
    public RestMetadataVdb createMetadataVdb(TeiidVdb teiidVdb, UnitOfWork uow, Repository repository, URI baseUri) throws Exception {
        KomodoObject vdbObj = repository.getObjectFactory().convertTeiidVdb(uow, repository, teiidVdb);
        WorkspaceManager mgr = WorkspaceManager.getInstance(repository, uow);
        Vdb vdb = mgr.resolve(uow, vdbObj, Vdb.class);
