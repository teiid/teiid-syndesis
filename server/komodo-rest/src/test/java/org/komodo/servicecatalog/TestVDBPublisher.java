...
public class TestVDBPublisher extends AbstractLocalRepositoryTest {
    ...
    public void testGenerateDeploymentYML() throws Exception {
        ...
        String javaOptions= " -Dswarm.datasources.data-sources.accounts-xyz.driver-name=postgresql "
                + "-Dswarm.datasources.data-sources.accounts-xyz.user-name=$(MYSECRECT-PG_USERNAME) "
                + "-Dswarm.datasources.data-sources.accounts-xyz.jndi-name=java:/accountsDS "
                + "-Dswarm.datasources.data-sources.accounts-xyz.password=$(MYSECRECT-PG_PASSWORD) "
                + "-Dswarm.datasources.data-sources.accounts-xyz.connection-url="
                + "jdbc:postgresql://$(PARAMETERS-PG_DATABASE_SERVICE_NAME):5432/$(MYSECRECT-PG_DATABASE_NAME)";

        assertThat(variables, hasItem(new EnvVar("PARAMETERS-PG_NAMESPACE", "openshift", null)));
        assertThat(variables, hasItem(new EnvVar("PARAMETERS-PG_VOLUME_CAPACITY", "1Gi", null)));
        assertThat(variables, hasItem(new EnvVar("PARAMETERS-PG_DATABASE_SERVICE_NAME", "postgresql", null)));
        assertThat(variables, hasItem(new EnvVar("PARAMETERS-PG_VERSION", "9.5", null)));
        assertThat(variables, hasItem(new EnvVar("PARAMETERS-PG_POSTGRESQL_DATABASE", "sampledb", null)));
        assertThat(variables, hasItem(new EnvVar("PARAMETERS-PG_MEMORY_LIMIT", "512Mi", null)));
        assertThat(variables, hasItem(new EnvVar("MYSECRECT-PG_DATABASE-USER", null,
                new EnvVarSource(null, null, null, new SecretKeySelector("database-user", "mysecrect-pg", null)))));
        assertThat(variables, hasItem(new EnvVar("MYSECRECT-PG_DATABASE-NAME", null,
                new EnvVarSource(null, null, null, new SecretKeySelector("database-name", "mysecrect-pg", null)))));
        assertThat(variables, hasItem(new EnvVar("MYSECRECT-PG_DATABASE-PASSWORD", null,
